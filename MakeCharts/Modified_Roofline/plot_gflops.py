...
# Questo dizionario deve diventare dinamico. In base al numero di core utilizzati 
# deve restituire quali sono le prestazioni massime (usando 0.167 FLOPs/Byte)
MAX_PERF = {
    1: 66,
    2: 133,
    3: 133,
...
if __name__=='__main__':
    ...
    # Plot subplots
    fig, ax = plt.subplots(len(ORDERS), 1, sharey=True, figsize=(15,15))
    ...
    for i, order_name in enumerate(ORDERS):
        ...
        # Max Performance L1
        max_performance_L1 = {k:blackyeti_perf.L1_interpolation(v['n-cores']) for k,v in ANALYSIS['ICC-vec'][order_name].items()}
        ...
        # Max Performance L2
        max_performance_L2 = {k:blackyeti_perf.L2_interpolation(v['n-cores']) for k,v in ANALYSIS['ICC-vec'][order_name].items()}
        ...
        # Max Performance L3
        max_performance_L3 = {k:blackyeti_perf.L3_interpolation(v['n-cores']) for k,v in ANALYSIS['ICC-vec'][order_name].items()}
        ...
        max_performance_DRAM = {k:blackyeti_perf.DRAM_interpolation(v['n-cores']) for k,v in ANALYSIS['ICC-vec'][order_name].items()}
    ...
    #Legend
    ax[0].legend(loc='upper left', ncol=(len(ANALYSIS)+4), bbox_to_anchor=(0, 1.15))
